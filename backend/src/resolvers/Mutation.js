...
const mutations = {
  ...
  async changePassword(parent, args, ctx, info) {
    ...
    if (args.newPassword === args.oldPassword) {
      throw new Error(
        "You cannot make your new password the same as your current one!"
      );
    }
  ...
  async createGameFilter(parent, args, ctx, info) {
    ...
    if (filter.game === "" && !filter.isGlobal) {
      throw new Error(
        "You must choose a game to assign the filter to, or set it as a global filter."
      );
    }
  ...
  async deleteGameNote(parent, args, ctx, info) {
    ...
    return ctx.db.mutation.deleteGameNote(
      {
        where: {
          id
        }
  ...
  async deletePlayerNote(parent, args, ctx, info) {
    ...
    const note = await ctx.db.query.playerNote(
      { where },
      `{id note user { id }}`
    );
